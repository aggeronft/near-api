...
module.exports = {
  ...
  View: async function (recipient, method, params, rpc_node, headers) {
    try {
      ...
      return await account.viewFunction(recipient, method, params);
    } catch (e) {
      return api.reject(e);
    }
  },
  ...
  GetBalance: async function (account_id) {
    try {
      ...
      return fetch(settings.rpc_node, {
        ...
        .then((res) => res.json())
        .then((json) => {
          if (json.error) return api.reject(json.error.data);
  ...
  DeployContract: async function (account_id, private_key, contract_file) {
    try {
      ...
      if (!fs.existsSync(path)) return api.reject("Contract not found");
